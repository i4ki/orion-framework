 ...
 * :: -------------------------------------------------------------
 * :: Opcionais ::
 * :: País - Armazena o país de origem do IP
 * :: Operadora - Armazena a empresa responsavel pelo IP  -- Ainda não ¬¬
 *
 */

class Security_Ident {
 	protected $user;
 	protected $ip;
 	...
 	public function getHTTP_REFERER() {
 		return isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : "Acesso direto ao site";
 	}
 	...
 	public function saveAtack() {
 		...
 		$str = '';
 		if( is_writable( Orion::getAttribute(Orion::ATTR_DIR_LOGS) ) === true ) {
 			$str .= "###########################################################\r\n";
 		...
 		$fp = fopen(Orion::getAttribute(Orion::ATTR_DIR_LOGS) . $file, 'a+');
 		fwrite($fp,$str);
 	}
